...
list_memory::list_memory(size_t size,
                         memory::allocate_mode allocation_mode,
                         logger* logger_use,
                         memory* another_allocator)
{
    ...
    size_t total_size = size + sizeof(logger*) + sizeof(memory*) + sizeof(size_t) + sizeof(void*) + sizeof(allocation_mode);
    try
    {
    ...
    *logger_place = logger_use;
    ...
    memory::allocate_mode *mode_alloc = reinterpret_cast<memory::allocate_mode*>(logger_place + 1);
...
{
    return sizeof(size_t) + sizeof(memory *) + sizeof(logger *) + sizeof(allocate_mode) + sizeof(void *);
}
...
void* list_memory::allocate(size_t block_size) const{
    ...
    memory::allocate_mode current_alloc_mode = *reinterpret_cast<memory::allocate_mode*>(reinterpret_cast<unsigned char*>(_memory)+sizeof(size_t)+sizeof(memory*)+sizeof(logger*));
    ...
    while (current_aviable_block != nullptr)
    {
        ...
        {
            if (current_alloc_mode == memory::allocate_mode::first_fit ||
                current_alloc_mode == memory::allocate_mode::best_fit && (target_block == nullptr || curr_sz < get_current_aviable_block_size(target_block)) ||
                current_alloc_mode == memory::allocate_mode::worst_fit && (target_block == nullptr || curr_sz > get_current_aviable_block_size(target_block)))
            {
...
void list_memory::dump_allocator_state(bool is_allocate) const noexcept
{
    ...
    auto *current_any_block = reinterpret_cast<unsigned char *>(_memory) + sizeof(size_t) + sizeof(memory*) + sizeof(logger *) + sizeof(allocate_mode) + sizeof(void *);
