#include "alloc_global_heap.h"
#include <iostream>
#include <stddef.h>

alloc_global_heap::alloc_global_heap(
    logger *logger)
    : _logger(logger)
{
...
size_t alloc_global_heap::get_occupied_block_size(
...
void *alloc_global_heap::allocate(size_t block_size) const
{
    auto got_typename = get_typename();
...
void alloc_global_heap::deallocate(void * const block_pointer) const
{
    auto got_typename = get_typename();
...
void alloc_global_heap::setup_allocation_mode(memory::allocate_mode mode)
{
    auto got_typename = get_typename();
...
logger *alloc_global_heap::get_logger() const
{
    return _logger;
}

std::string alloc_global_heap::get_typename() const noexcept
{
